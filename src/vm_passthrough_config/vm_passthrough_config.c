...
static bool
mmio_range_access_valid(pgtable_access_t mmio_access,
			pgtable_access_t pvm_access)
{
	bool ret;

	switch (pvm_access) {
	case PGTABLE_ACCESS_RW:
		ret = (mmio_access == PGTABLE_ACCESS_RW) ||
		      (mmio_access == PGTABLE_ACCESS_R);
		break;
	case PGTABLE_ACCESS_R:
		ret = (mmio_access == PGTABLE_ACCESS_R);
		break;
...
vm_passthrough_config_unmap_ioranges(const rm_env_data_t *env_data)
{
	...
	cap_id_t device_me = vm_memory_get_owned_extent(hlos_vm, MEM_TYPE_IO);

	vm_memory_batch_start(device_me);
	for (index_t i = 0; i < env_data->device_assignments->num_devices;
	     i++) {
		if (vm_passthrough_device_get_vmid(i) == VMID_HLOS) {
			continue;
		}

		vm_device_descriptor_t *device =
			&env_data->device_assignments->devices[i];
		for (index_t j = 0; j < device->num_mmio_ranges; j++) {
			bool non_exclusive   = false;
			bool is_mapped	     = true;
			bool is_overlap_hlos = false;

			root_env_mmio_range_descriptor_t io_range =
				device->mmio_ranges[j];
			if (root_env_mmio_range_properties_get_pvm_unmapped(
				    &io_range.attrs)) {
				// Range is not mapped in PVM.
				continue;
			}
			...
			is_mapped = (ret.err == OK);

			if (!is_mapped) {
				(void)printf(
					"Warning: passthrough config lookup of %lx %lx failed %d\n",
					ipa, size, ret.err);
#if defined(PLATFORM_IOMEM_STATIC_SHARE_ALLOWED) &&                            \
	(PLATFORM_IOMEM_STATIC_SHARE_ALLOWED != 0)
				(void)printf("static share %lx %lx\n", ipa,
					     size);
#else
				(void)printf(
					"remove %lx %lx from passthrough\n",
					ipa, size);
				device->mmio_ranges[j].address =
					INVALID_ADDRESS;
				continue;
#endif
			}

			pgtable_access_t mmio_access =
				root_env_mmio_range_properties_get_access(
					&io_range.attrs);
			if (!mmio_range_access_valid(mmio_access, ret.access)) {
				(void)printf(
					"VM passthrough: invalid MMIO access %d %d\n",
					mmio_access, ret.access);
				device->mmio_ranges[j].address =
					INVALID_ADDRESS;
				continue;
			}
			...
			// For now we assume the overlapping VM is always HLOS.
			non_exclusive =
			...
			// Check if this address range overlaps HLOS
			is_overlap_hlos =
				vm_passthrough_config_is_addr_in_range(
					VMID_HLOS, ipa, size,
					PGTABLE_ACCESS_NONE, false);
			if (is_overlap_hlos) {
				(void)printf("overlap %lx %lx HLOS\n", ipa,
					     size);
			}

			if (is_mapped && !is_overlap_hlos && !non_exclusive) {
				size_t offset = io_range.address;
				err	      = vm_memory_unmap_partial(hlos_vm,
									VM_MEMUSE_IO,
									device_me, ipa,
									offset, size);
				if (err != OK) {
